...
def compute_weights(labels, classes, count, verbose=False):#classes是每个物体在所有scan中出现的次数总数，count是所有边的数量
    if verbose: print("-------------")    
    sum_weights = 0
...
def compute(classNames,relationNames, relationship_data, selections:list = None, verbose=False):
    ...
    for scan in relationship_data['scans']:
        ...
        for relationship in scan["relationships"]:
            ...
            nn = str(obj)+'_'+str(sub) #nn记录边，nnk是边的汇总
            if nn not in nnk:
                nnk[nn] = 0
    ...
    wobjs = compute_weights(classNames, o_obj_cls, classes_count,verbose)#将20个物体类别的权重归一化；o_obj_cls是在所有scan中，物体出现的次数统计；classes_count统计所有scan中，边的数量；
    if verbose: print("relationships:")
    wrels = compute_weights(relationNames, o_rel_cls, classes_count,verbose)#将7个关系的权重归一化，o_rel_cls是在所有scan中，边出现的次数统计
    return wobjs,wrels,o_obj_cls,o_rel_cls #归一化的权重，权重，数据集中物体各类别的数量，数据集中关系的数量
