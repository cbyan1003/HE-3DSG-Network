...
class Gen_edge_descriptor(MessagePassing):#TODO: move to model
    ...
    def forward(self, descriptor, input_data, edges_indices):
        size = self.__check_input__(edges_indices, None)
        coll_dict = self.__collect__(self.__user_args__,edges_indices,size, {"x":input_data})#{"x":torch.cat((descriptor, input_data), dim = 1)})
        msg_kwargs = self.inspector.distribute('message', coll_dict)
        edge_feature = self.message(**msg_kwargs) # 字典解包
        return edge_feature
    ...
    def message(self, x_i, x_j):
        ...
        edge_feature[:,0:64] = x_i[:,0:64]
        edge_feature[:,64:128] = x_j[:,64:128]
        edge_feature[:,128:192] = x_i[:,128:192] - x_j[:,128:192]
        edge_feature[:,192:256] = (x_i[:,192:256] + x_j[:,192:256])/2
        edge_feature[:,256:320] = torch.mul(x_i[:,256:320], x_j[:,256:320])
        edge_feature[:,320:384] = torch.log(x_i[:,320:384]/ x_j[:,320:384])
        edge_feature[:,384:448] = torch.max(torch.stack([x_i[:,384:448], x_j[:,384:448]], dim = 1), dim = 1).values
        edge_feature[:,448:512] = torch.min(torch.stack([x_i[:,448:512], x_j[:,448:512]], dim = 1), dim = 1).values
        ...
        # edge_feature[:,11:75] = x_i[:,11:75]
        # edge_feature[:,76:140] = x_j[:,76:140]
        # edge_feature[:,141:205] = x_i[:,141:205] - x_j[:,141:205]
        # edge_feature[:,206:270] = (x_i[:,206:270] + x_j[:,206:270])/2
        # edge_feature[:,271:335] = torch.mul(x_i[:,271:335], x_j[:,271:335])
        # edge_feature[:,336:400] = torch.log(x_i[:,336:400]/ x_j[:,336:400])
        # edge_feature[:,401:465] = torch.max(torch.stack([x_i[:,401:465], x_j[:,401:465]], dim = 1), dim = 1).values
        # edge_feature[:,466:530] = torch.min(torch.stack([x_i[:,466:530], x_j[:,466:530]], dim = 1), dim = 1).values
