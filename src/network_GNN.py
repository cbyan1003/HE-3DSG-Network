...
class GraphEdgeAttenNetwork(BaseNetwork):
    ...
    def forward(self, x, edge_feature, edge_index):
        ...
        x_i, x_j = self.index_get(x, edge_index) # 根据边，找出xi xj两个点
        xx, gcn_edge_feature, prob = self.edgeatten(x_i,edge_feature,x_j) # 边注意力机制生成边的GCN特征
        xx = self.index_aggr(xx,edge_index, dim_size = x.shape[0]) # 消息聚合xx
